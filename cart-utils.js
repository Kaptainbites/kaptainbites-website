const KAPTAINBITES_CART_STORAGE_KEY = "kaptainbites-cart-v1";
...
if (!items.length) {
return "Hi, I want to know more about KaptainBites products.";
}

const orderLines = items.map((item, index) => {
return `${index + 1}. ${item.product.name} x ${item.quantity} = Rs.${item.lineTotal}`;
});
